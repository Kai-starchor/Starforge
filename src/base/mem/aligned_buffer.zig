...
test "AlignedBuffer - originalToAligned and alignedToOriginal are consistent" {
    inline for ([_]struct {
        ...
        const original_len = AlignedBuffer.alignedToOriginal(test_case.aligned_len, test_case.alignment);
        const aligned_len = AlignedBuffer.originalToAligned(original_len, test_case.alignment);
